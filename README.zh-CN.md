...
负责 Interval Type-2 Sugeno 模糊推理与进化优化，Python 负责数据协议、无泄漏预处理、
基线模型、实验编排、命令行和 Streamlit 本地界面。
...
没有 MATLAB 时仍可校验数据、运行可用的 Python 基线并浏览已有结果；Streamlit 会自动
进入预计算结果浏览模式。
...
契约和 `matlab -batch` 调用，不依赖 MATLAB Engine。
...
每次 benchmark 都生成独立 artifact 目录，包括配置、数据 SHA-256、指标、预测，以及
MATLAB 可用时的 `.mat` 模型、规则表和激活矩阵。
